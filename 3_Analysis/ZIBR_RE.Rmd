...
sim.list$sim.data <- lapply(true.par,function(true.p){do.call(simulate_zero_inflated_beta_random_effect_data,true.p)})
...
if (file.exists(sim.temp.file)) {
  ...
  if (identical(sim.list.temp$sim.data, sim.list$sim.data, ignore.environment = TRUE)) {
    if (exists("zibr",where=sim.list.temp)) &
      ...
      if (FALSE) {
        sim.list$zibr <- lapply(sim.data,
                                function(sdata) {
                                  do.call(run_zibre,sdata);
                                })
      }else{
        sim.list$zibr <- mclapply(sim.data,
                                  function(sdata) {
                                    do.call(run_zibre,sdata);
                                  },mc.cores = 5)
      }
