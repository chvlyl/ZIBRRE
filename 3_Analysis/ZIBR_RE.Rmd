...
list_to_vector = function(est){
  res.vec <- c(as.vector(as.vector(est$logistic.est.table)),
               ...
               est$beta.v.est,
               est$overall.test.p)
  names(res.vec) <- c(paste('logistic',rownames(est$logistic.est.table),'coef',sep='.'),
                      paste('logistic',rownames(est$logistic.est.table),'pval',sep='.'),
...
for (subject.n in sub.N){
  for (time.n in time.N){
    ...
    for (sim.n in 1:sim.N){
      ...
      Y <- as.matrix(Y)
      Z <- as.matrix(Z)
      #### beta regression with random effect
      est <- fit_beta_random_effect(Z=Z,Y=Y,
                   subject.ind=subject.ind,
                   time.ind=time.ind,
                   quad.n=50,verbose=TRUE)
      est.list[[paste('sim',sim.n,sep='')]] <- as.vector(est$est.table)
...
#write.csv(est.avg.mat,file=paste('../4_Results/Simulation_Parameter_Estimation_a_n05_1_b_neg05_05_s1_s2_1_v_5_Mean.csv',sep=''))
#write.csv(est.var.mat,file=paste('../4_Results/Simulation_Parameter_Estimation_a_n05_1_b_neg05_05_s1_s2_1_v_5_Var.csv',sep=''))
