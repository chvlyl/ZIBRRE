...
for (spe in spe.all){
  ...
  add_rownames(as.data.frame(abu),var='Sample') %>% 
  left_join(add_rownames(sample.info,var='Sample'),by='Sample') %>% 
  dplyr::select(Taxa,log.FCP,Time,Subject,Treatment.Specific,Response) %>%
  dplyr::mutate(dummy=factor(1)) %>%
  (function(pdata){
  p<- pdata %>% dplyr::filter(Taxa>0) %>%
  #cor(log10(p$Taxa),p$log.FCP,method='spearman')
  ggplot(aes(log.FCP,log(Taxa/(100-Taxa)),colour=factor(Subject),group=factor(Subject)))+
  geom_point() + geom_line() + ggtitle(spe) + facet_grid(Treatment.Specific ~ Response)
  #facet_grid(.~Response, margins=TRUE) +
  #ylim(c(0,5))
  #print(p)
  plot(p)
  ...
  add_rownames(as.data.frame(abu),var='Sample') %>% 
  left_join(add_rownames(sample.info,var='Sample'),by='Sample') %>% 
  dplyr::select(Taxa,log.FCP,Time,Subject) %>%
  dplyr::filter(Taxa>0) %>% as.data.frame
}
```
